...
## 2. 自動產生的帳在清單裡看不出來（已完成）

**已做**：`Row` 在有 `recurringId` 的帳旁邊加了細框淡字的「定期」標記，沒有上色。
以下保留當初的評估。

**原本的現況**：定期支出確認後寫入的交易有 `recurringId`，
...
**已做**：`MonthSummary` 的「支出」「收入」兩欄可以點，點了就切換下方排行看哪一種，
被選的那欄標籤加底線。沒有另開常駐區塊，頁面長度不變。收入那邊沒有帳時會明講
「沒有收入的帳」，不然點了什麼都沒出現會像壞掉。以下保留當初的評估。

**原本的現況**：`CategoryRank` 固定吃支出。
