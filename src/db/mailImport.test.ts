...
describe('ingestMailImports', () => {
  ...
  test('已經存在的不論狀態都不覆寫，略過的不會跑回來', async () => {
    ...
    expect(stored?.amountMinor).toBe(20500)
  })
  ...
  test('新到舊，同一天看收信時間；確認過與略過的不列', async () => {
    await ingestMailImports(
      [
        ...
        row('late', { date: '2026-09-18', receivedAt: 20 }),
        row('early', { date: '2026-09-18', receivedAt: 10 }),
        row('done'),
        row('skip'),
      ],
      1,
...
describe('confirmMailImport', () => {
  test('寫成一筆支出，備註是 label，並累加未備份筆數', async () => {
    await ingestMailImports([row('a', { label: '轉帳：雞排' })], 1, [])
  ...
  test('每個 label 取最近一次確認時選的分類', async () => {
    await ingestMailImports(
      [
        row('a', { receivedAt: 1 }),
        row('b', { receivedAt: 2 }),
        row('c', { label: '繳費：台新銀行', receivedAt: 3 }),
      ],
      1,
      [],
    )
    ...
    expect(hints.get('繳費：台新銀行')).toBe('fixed')
  })
})
