...
轉入帳號 ：0000*****11111 轉帳金額 ：1,145 手續費 ：0 交易序號 ：00000001
附言(給自己) ： 附言(給對方) ：雞排 完成時間 ：115年09月15日13時03分28秒
如有任何疑問，請撥打本公司24小時顧客服務專線。`

const BILL_BODY = `親愛的客戶您好： 您使用本公司存簿儲金帳戶繳費交易結果如下：
交易存簿儲金帳號：0000*****00000 轉入行庫：812 / 台新銀行
轉入帳號：000000000****000 完成時間：115/08/23 20:26:40
交易金額：1290 元 手續費：10.0 元`
...
describe('行動郵局轉帳', () => {
  ...
  test('兩個附言都空的時候只寫轉帳', () => {
    const body = TRANSFER_BODY.replace('：雞排', '：')
...
完成時間 ：115年09月01日08時00分00秒`
  ...
  test('手續費算進金額：那也是從帳戶出去的錢', () => {
    ...
    expect(result?.amountMinor).toBe(116000)
    expect(result?.label).toBe('轉帳：雞排（含手續費 15）')
  })
})
...
<tr><td>交易日期：2026年09月18日12:49:26</td></tr>
<tr><td>交易平台：連加電子支付(帳號：00*****00000)</td></tr>
<tr><td>交易金額：1,205&nbsp;元</td></tr></table>`
